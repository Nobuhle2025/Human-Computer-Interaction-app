...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect('login')  # Redirect back to the login page if invalid
...
def schedule_view(request):
    schedules = ClassSchedule.objects.filter(user=request.user)  # Fetch classes for the current user

    if request.method == "POST":
        form = ClassScheduleForm(request.POST)
        if form.is_valid():
            new_class = form.save(commit=False)
            new_class.user = request.user  # Associate class with the current user
            new_class.save()
            return redirect('schedule')  # Redirect back to the schedule page after saving
...
def wellness_view(request):
    if request.method == 'POST':
        ...
        return redirect('wellness')  # Redirect to avoid resubmission on refresh
